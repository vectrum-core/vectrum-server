...
const log = require('../../../../logger').getLogger('ROUTER: "/api/v1/auth"');
...
router.post('/tg',
  async (req, res, next) => {
    ...
    if (checkTelegramAuthData({ token, ...authData })) {
      ...
      log.debug('%s Login by telegram successed!', logTrace, SS(req.body));
    } else
      log.warn('%s Login by telegram failed!', logTrace, SS(req.body));
